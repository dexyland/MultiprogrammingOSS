	...
	private void checkJobs(){
		if (this.queueAvailable) {
			...
			if (!this.priorityQueue0.isEmpty()) {
				...
				for (int i = 0; i < size; i++) {
					if (this.priorityQueue1.get(i).getElapsedTime() > offset + 5 || this.priorityQueue0.get(i).getElapsedTime() < 9) {
						this.priorityQueue2.add(this.priorityQueue1.remove(i));
					}
				}
			} else if (!this.priorityQueue2.isEmpty()) {
			} else {
				///System.out.println("Priority queues are empty. No jobs to check!");
			}
		} else {
			System.out.println("Access to queues denied!");
		}
